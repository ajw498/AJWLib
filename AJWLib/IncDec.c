...
static void AJWLib_Icon__ResetCaret(const Desk_window_handle window, const Desk_icon_handle icon)
{
...
{
 int i;
 if ((i=Desk_Icon_GetInteger(b->window, b->texticon))<(b->max-(signed int)b->step))
                               Desk_Icon_SetInteger(b->window, b->texticon, i+(signed int)b->step);
...
AJWLib_Icon__ResetCaret(b->window, b->texticon);
}
...
{
 int i;
 if ((i=Desk_Icon_GetInteger(b->window, b->texticon))>(b->min+(signed int)b->step))
                               Desk_Icon_SetInteger(b->window, b->texticon, i-(signed int)b->step);
